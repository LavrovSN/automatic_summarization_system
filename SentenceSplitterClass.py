...
class NormalizerDE(object):
    ...
    def lemmatize(self, word, lexicon):
        ...
        """
	Метод лемматизации немецких слов.
	
	Подгружается особым образом сформированный лексикон 
	(удалены сущ.омонимы, лемма для форм причастия - глагол).
	...
	значение - список словоформ этой леммы. Список отсортирован по частотности,
	т.е. первой в списке стоит словоформа которая чаще всего встречается в языке.
	Выделение трех букв сделано для того, чтобы как-то уменьшить время поиска словоформы. 
	Так не надо пробегать по всему словарю, а искать только в одном подсловаре.
	Поиск:
	...
	которых нет в лексиконе, но есть их главная часть (bescheinigungen).
	"""
